...
class Witcher(Unit):
    ...
    def next_move(self, history: list, turn: int, enemies_info: list, player_action, message) -> tuple:
        # print(reduce(lambda x, y: str(x) + '\n' + str(y), enemies_info))
        # print('номер хода: ' + str(turn))
        playerAction = None
        damage = 0
        try:
            # player_action = int(input('выберите следующее действие'))
            ...
            if player_action == 2:
                ...
                if self.last_turn_power_attack == -1 or turn - self.last_turn_power_attack >= 3:
                    ...
                    if self.last_turn_potions_used[Potions.thunder] != -1 and turn - self.last_turn_potions_used[
                        ...
                        message['log'].append('Ведьмак использовал силовую атаку. HP ведьмака {0}'.format(self.hp))

                    else:
                        damage = self.attack_power * 2
                    # break
                else:
                    message['error_code'] = 'Вы не можете провести силовую атаку, введите действие еще раз'
                    return None, 0
            ...
            if player_action == 3:
                ...
                damage = 0
                message['log'].append('Ведьмак использовал уклонение {0}, HP ведьмака {1}'.format(turn, self.hp))
                # break
            ...
            if player_action == 4:
                if self.potions[Potions.thunder] != 0:
                    ...
                    message['log'].append('{0}, {1}, использован Гром'.format(turn, self.hp))
                else:
                    message['error_code'] = 'У вас нет зелья Гром. Введите действие еще раз'
                    return None, 0
            ...
            if player_action == 5:
                if self.potions[Potions.swallow] != 0:
                    ...
                    message['log'].append('{0}, {1}, использована Ласточка'.format(turn, self.hp))
                else:
                    message['error_code'] = 'У вас нет зелья Ласточка. Введите действие еще раз'
                    return None, 0
            ...
            if player_action == 8:
                ...
                if self.energy == 100:
                    damage: int = 0
                    self.energy -= 85
                    message['log'].append('Ход {0}.Использован Игни. ХП Ведьмака {1}'.format(turn, self.hp))
                    # break
                else:
                    message['error_code'] = 'недостаточно энергии на использование знака'
                    return None, 0
            ...
            if player_action == 10:
                ...
                if self.energy == 100:
                    damage = 0
                    self.energy -= 90
                    message['log'].append('Ход {0}.Использован Аксий. ХП Ведьмака {1}'.format(turn, self.hp))
                    # break
                else:
                    message['error_code'] = 'недостаточно энергии на использование знака'
                    return None, 0
            if player_action == 11:
                if self.potions[Potions.tawny_owl]!= 0:
                    ...
                    message['log'].append('{0}, {1}, использована Неясыть'.format(turn, self.hp))
                else:
                    message['error_code'] = 'У вас нет зелья Неясыть. Введите действие еще раз'
                    return None, 0   
        except ValueError:
        ...
        if self.last_turn_potions_used[Potions.swallow] != -1 and turn - self.last_turn_potions_used[
            ...
            message['log'].append('Ход {0}.ласточка захилила +{1} хп'.format(turn, abs(previous_hp - self.hp)))
        ...
        if self.last_turn_potions_used[Potions.tawny_owl]!=-1 and turn-self.last_turn_potions_used[Potions.tawny_owl] < 5:
                ...
                message['log'].append('Ход {0}. Неясыть восстановила +{1} энергии'.format(turn, abs(previous_energy - self.energy)))
    ...
    def witcher_main_damage(self, damage, turn, history, critical_hit, message):
        if self.shield > 0:
            if damage > self.shield:
                diff_damage = damage - self.shield
                diff_shield = damage - diff_damage
                self.shield = self.shield - diff_shield
                message['log'].append('Ход {0}. Противник попал по щиту. Щит поглотил {1} урона'.format(
                ...
                message['log'].append('Ход {0}. Противник пробил щит и нанес Ведьмаку {1} урона'.format(
                    turn, diff_damage))
                message['log'].append('Ход {0}. Противник пробил щит и нанес Ведьмаку {1} урона'
            ...
            if critical_hit:
                ...
                message['log'].append('Ход {0}. Противник нанес критический урон {1} по Ведьмаку'.format(turn, int(damage)))
                history.append(
